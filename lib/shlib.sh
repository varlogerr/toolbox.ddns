...
print_help() {
  text_decore "
   ...
   .
    Usage:
    $(text_decore "${HELP_USAGE}" | sed -E 's/^/.  /g')
   .
    Available options:
    $(text_decore "${HELP_OPTS}" | sed -E 's/^/.  /g')
   .
    Environment variables can be used
    instead of inline arguments.
    Supported environment variables:
    $(text_decore "${HELP_ENVVARS}" | sed -E 's/^/  * /g')
   .
    Demo:
    $(text_decore "${HELP_DEMO}" | sed -E 's/^/.  /g')
  "
}

eval "$(typeset -f _log_type | sed '1s/ \?(/_overriden_ (/')"
_log_type() {
  LOG_TOOLNAME+="${LOG_TOOLNAME:+ "($(get_ts))"}" _log_type_overriden_ "${@}"
}
